...
def image_3_clicked(event=None):
    ...
    if entry_1_value>totalQues:
        messagebox.showerror("Error","Invalid Question No.")
        return
    querry = f"delete from {quizName} where qno=%s"
    querryWritter.execute(querry, (entry_1_value,))
    querry=f"update {quizName} set qno=%s where qno=%s"
    for i in range(entry_1_value+1,totalQues+1):
        querryWritter.execute(querry,(i-1,i))
        conn.commit()
    conn.commit()
    messagebox.showinfo("Done","Question Deleted Successfully")
...
image_image_3 = PhotoImage(file=("Images\\delete_ques_dash1.png"))
